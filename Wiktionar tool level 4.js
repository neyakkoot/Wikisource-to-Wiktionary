// ============================================================
// Wikisource to Wiktionary Entry Importer (Append if Exists Fix)
// ============================================================
...
(function () {
    ...
    function showInteractiveModal() {
        ...
        const wordGroup = $('<div>').css('margin-bottom', '10px').append($('<label>').css({'font-weight': 'bold', 'display': 'block', 'margin-bottom': '4px', 'font-size': '13px'}).text('தலைப்புச் சொல்:'),
            $('<input>', { type: 'text', id: 'input-word' }).css({
                'width': '100%', 'padding': '6px 8px', 'border': '1px solid #ccc', 'border-radius': '4px', 'box-sizing': 'border-box'
            })
        );

        const meaningGroup = $('<div>').css('margin-bottom', '10px').append($('<label>').css({'font-weight': 'bold', 'display': 'block', 'margin-bottom': '4px', 'font-size': '13px'}).text('பொருள்/விளக்கம்:'),
        ...
        const btnGroup = $('<div>').css({'display': 'flex', 'justify-content': 'space-between', 'align-items': 'center'}).append($('<button>', { id: 'skip-word-btn', text: 'தவிர் (Skip)' }).css({
    ...
    // 3. FOOYYA'INSA: `createonly: true` kaafamuu fi jecha jiraatu haaromsuu
    async function submitCurrentWord() {
        ...
        $('#status-msg').text('விக்சனரியில் பக்கம் சரிபார்க்கப்பட்டு உருவாக்கப்படுகிறது...').css('color', '#00af89');

        try {
            // Fuula jiraachuu fi jiraachuu dhiisuu isaa sakatta'uu
            ...
            if (page && !page.missing) {
                // Fuulichi duraan yoo jiraate, hiika haaraa sana ida'uu
                ...
                // Fuulichi yoo hin jire ta'e foormatii guutuu uumuu
                finalContent = buildWiktionaryPageContent(word, meaning);
            }

            // Wiktionary irratti galmeessuu
            ...
            if (editRes && editRes.edit && editRes.edit.result === 'Success') {
                ...
                $('#status-msg').html('வெற்றி: <a href="' + url + '" target="_blank" style="color:green;">' + word + '</a> உருவாக்கப்பட்டது/புதுப்பிக்கப்பட்டது!').css('color', 'green');
