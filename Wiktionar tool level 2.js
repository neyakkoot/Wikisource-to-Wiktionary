// ============================================================
// Wikisource to Wiktionary Entry Importer (Fixed Version)
// 
...
(function () {
    ...
    // 3. முகப்பு (Modal UI) சரிசெய்யப்பட்ட வடிவம்
    function showInteractiveModal() {
        ...
        const dialog = $('<div>').css({
            ...
            'overflow-y': 'auto', 'padding': '24px', 'border-radius': '8px',
        ...
        const title = $('<h2>').css({
            'margin-top': '0', 'font-size': '18px', 'border-bottom': '1px solid #ccc', 'padding-bottom': '8px'
        }).html('விக்சனரியில் புதிய பக்கம் உருவாக்குதல் (<span id="current-step-num">1</span>/' + uncreatedWordsList.length + ')');

        const wordGroup = $('<div>').css('margin-bottom', '12px').append(
            $('<label>').css({'font-weight': 'bold', 'display': 'block', 'margin-bottom': '4px'}).text('தலைப்புச் சொல்:'),
            $('<input>', { type: 'text', id: 'input-word' }).css({
                'width': '100%', 'padding': '8px', 'border': '1px solid #ccc', 'border-radius': '4px', 'box-sizing': 'border-box'
            })
        );

        const meaningGroup = $('<div>').css('margin-bottom', '12px').append(
            $('<label>').css({'font-weight': 'bold', 'display': 'block', 'margin-bottom': '4px'}).text('பொருள்:'),
        ...
        const previewDetails = $('<details>', { open: true }).css({
            'font-size': '12px', 'background': '#f8f9fa', 'padding': '8px', 'border-radius': '4px', 'margin-bottom': '12px'
        }).append(
            ...
            $('<pre>', { id: 'wikitext-preview' }).css({
                ...
                'max-height': '180px', 'overflow-y': 'auto', 'background': '#fff', 'padding': '6px', 'border': '1px solid #eee'
            })
        );
        ...
        const btnGroup = $('<div>').css({'display': 'flex', 'justify-content': 'space-between', 'align-items': 'center'}).append(
            $('<button>', { id: 'skip-word-btn', text: 'தவிர் (Skip)' }).css({
                'padding': '8px 16px', 'background': '#72777d', 'color': '#fff', 'border': 'none', 'border-radius': '4px', 'cursor': 'pointer'
            }).click(function() { skippedCount++; nextWord(); }),
            $('<button>', { id: 'submit-word-btn', text: 'பக்கத்தை உருவாக்கு' }).css({
                'padding': '8px 16px', 'background': '#36c', 'color': '#fff', 'border': 'none', 'border-radius': '4px', 'cursor': 'pointer', 'font-weight': 'bold'
            }).click(submitCurrentWord)
        );

        const statusMsg = $('<div>', { id: 'status-msg' }).css({
            'margin-top': '10px', 'font-weight': 'bold', 'font-size': '13px', 'text-align': 'center'
        });
